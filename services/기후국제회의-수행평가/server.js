...
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='GET'&&url.pathname==='/api/project-info')return sendJson(res,200,{projectId:PROJECT_ID,name:PROJECT_NAME,version:APP_VERSION,type:'server',studentPath:'/',teacherPath:'/teacher',operatePath:'/operate',theme:'green',mobileOptimized:true,totalActiveMinutes:40,phaseGate:false,teacherPause:false,serverGate:true,rosterUpload:true,aiGrading:true,teacherApiKeyInput:true});
  ...
  if(req.method==='POST'&&url.pathname==='/api/start'){
   ...
   const rt=await readRuntime(),className=rosterEntry?.className||deriveClass(studentId),classNo=Number(String(className).replace(/\D/g,''));if(classNo<1||classNo>7)return sendJson(res,409,{error:'학생의 반 정보를 확인할 수 없습니다. 교사에게 문의하세요.'});let {cr,rec}=await currentClassRecord(classNo);if(!rec||['closed','finished'].includes(rec.phase)){rec=classRecord(classNo,'regular');rec.phase='running';rec.runStartedAt=now();cr.sessions[rec.sessionId]=rec;cr.classes[String(classNo)]=rec.sessionId;await writeClassRuntime(cr);}else if(['ready','paused','auto_paused'].includes(rec.phase)){rec.phase='running';rec.runStartedAt=now();rec.checkpointSeconds=TOTAL_SECONDS;rec.updatedAt=now();cr.sessions[rec.sessionId]=rec;await writeClassRuntime(cr);}const country=rosterEntry?.country||fallbackCountry(studentId,className),pauseBaseMs=effectivePausedMs(rt),sessionId=newId();
