...
const BENCHMARK_FILE = path.join(DATA_DIR, 'benchmark-50.json');
const TEACHER_DEMO_CLEANUP_MARKER = path.join(DATA_DIR, '.teacher-demo-cleanup-v1');
...
const compromiseLabels={target:'감축률',money:'재정 지원',schedule:'이행 시기',rate:'감축률',finance:'재정 지원',time:'이행 시기',technology:'기술 지원',other:'기타 조건'};
const scoreOptions={countryAnalysis:[15,12,9],fundAllocation:[20,16,12],policyAgreement:[20,16,12],conflictAnalysis:[15,12,9],compromise:[10,8,6],cooperationRoles:[10,8,6],completion:[4,3,2],timeCompliance:[6,4,2,0]};
const scoreMax=Object.fromEntries(Object.entries(scoreOptions).map(([k,v])=>[k,Math.max(...v)]));
const scoreLabels={countryAnalysis:'국가 상황과 이해관계 분석',fundAllocation:'기후기금 배분의 타당성',policyAgreement:'정책·협약 선택과 근거',conflictAnalysis:'국가 간 이해관계와 갈등 분석',compromise:'갈등 조정 및 절충안',cooperationRoles:'국제협력 주체의 역할',completion:'활동 과정 및 과제 완성도',timeCompliance:'시간 준수'};
...
async function readStudentSessions(){return (await readSessions()).filter(s=>String(s.studentId)!==TEACHER_STUDENT_ID);}
...
function classElapsedSeconds(rec,at=Date.now()){let elapsed=Math.max(0,Math.floor(Number(rec?.elapsedSeconds||0)));if(rec?.phase==='running'&&rec.runStartedAt){const started=Date.parse(rec.runStartedAt);if(Number.isFinite(started))elapsed+=Math.max(0,Math.floor((at-started)/1000));}return Math.min(Number(rec?.timeLimitSeconds||TOTAL_SECONDS),elapsed);}
async function finalizeClassAnswers(rec){
 const sessions=await readSessions();
 for(const student of sessions.filter(x=>x.classSessionId===rec.sessionId&&!x.timerExempt&&x.status!=='submitted')){
  await appendEvent({type:'submit',sessionId:student.sessionId,data:{},submissionReason:rec.finishReason||'time_expired',elapsedSeconds:classElapsedSeconds(rec),timeComplianceScore:timeScore(classElapsedSeconds(rec))});
 }
}
function timeScore(elapsed){return elapsed<=2700?6:elapsed<=3000?4:elapsed<=3300?2:0;}
async function settleClassRecord(cr,rec){
 if(!rec)return rec;
 if(rec.phase==='running'&&classElapsedSeconds(rec)>=Number(rec.timeLimitSeconds||TOTAL_SECONDS)){
  rec.elapsedSeconds=Number(rec.timeLimitSeconds||TOTAL_SECONDS);rec.runStartedAt=null;rec.phase='finished';rec.finishReason='time_expired';rec.updatedAt=now();
  cr.sessions[rec.sessionId]=rec;await writeClassRuntime(cr);
 }
 if(rec.phase==='finished')await finalizeClassAnswers(rec);
 return rec;
}
async function settleAllClasses(){const cr=await readClassRuntime();for(const id of Object.values(cr.classes))await settleClassRecord(cr,cr.sessions[id]);}
...
async function writePresence(){presenceLastWrite=Date.now();await fsp.writeFile(PRESENCE_FILE,JSON.stringify(readPresence(),null,2),'utf8');}
async function removeCurrentTeacherDemoRecords(){
 if(fs.existsSync(TEACHER_DEMO_CLEANUP_MARKER))return;
 const teacherSessions=(await readSessions()).filter(s=>String(s.studentId)===TEACHER_STUDENT_ID);
 for(const s of teacherSessions)await appendEvent({type:'reset',sessionId:s.sessionId,reason:'teacher-demo-cleanup-v1'});
 const p=readPresence();
 if(Object.prototype.hasOwnProperty.call(p,TEACHER_STUDENT_ID)){delete p[TEACHER_STUDENT_ID];await writePresence();}
 await fsp.writeFile(TEACHER_DEMO_CLEANUP_MARKER,JSON.stringify({completedAt:now(),removed:teacherSessions.length})+'\n','utf8');
}
...
function applySessionEvent(sessions,e){
  ...
  if(e.type==='start')sessions.set(e.sessionId,{sessionId:e.sessionId,studentId:e.studentId,name:e.name,className:e.className||deriveClass(e.studentId),country:e.country,pauseBaseMs:Number(e.pauseBaseMs||0),timerMode:e.timerMode||'legacy',classNo:Number(e.classNo||0),classSessionId:e.classSessionId||null,startedAt:e.ts,updatedAt:e.ts,submittedAt:null,status:'in_progress',progress:0,data:{},score:null,scoreHistory:[],teacherNote:'',aiGrade:null,aiGradeHistory:[],gradeReviewRequired:false,synthetic:!!e.synthetic,timerExempt:!!e.timerExempt,reopenedAt:null,reopenCount:0,reopenHistory:[]});
  const s=sessions.get(e.sessionId);if(!s)return;s.revision=Number(s.revision||0)+1;
...
async function teacherExportData(className=''){
 const sessions=await readStudentSessions(),roster=await readRoster(),sessionById=new Map(sessions.map(s=>[String(s.studentId),s]));
 const base=roster.students.length?roster.students:sessions;
 const keys=Object.keys(scoreOptions);
 const headers=['반','학번','이름','국가','상태','진행률','시작시각','제출시각','채점상태','처음_우선가치','처음_판단이유','선택협약','예산_재생에너지지원','예산_기후재난지원','예산_친환경기술','예산_산림보호','예산_산업전환','협약과예산_선택이유','상대국가','절충조정요소','절충방식','절충안','협력주체','협력주체역할배정','협력이유','재판단','판단변화이유','최종합의문','서버경과초','서버_시간점수','제출확정사유','AI_가채점총점','AI_신뢰도','AI_종합판단','AI_잘한점','AI_보완할점','AI_교사용피드백','AI_교사확인사항',...keys.flatMap(k=>[`AI_${scoreLabels[k]}_점수`,`AI_${scoreLabels[k]}_근거`]),...keys.map(k=>`교사_${scoreLabels[k]}_점수`),'교사_최종총점','교사메모'];
 const rows=base.filter(r=>String(r.studentId)!==TEACHER_STUDENT_ID&&(!className||(r.className||deriveClass(r.studentId))===className)).map(r=>{
  const s=sessionById.get(String(r.studentId)),d=s?.data||{},b=d.budget||{},ai=s?.aiGrade||{},sc=s?.score||{};
  return [r.className||deriveClass(r.studentId),String(r.studentId),r.name,countryLabels[s?.country||r.country]||'',s?koStatus(s.status):'미시작',s?.progress??0,s?.startedAt||'',s?.submittedAt||'',s?.gradeReviewRequired?'재채점 필요':s?.score?'채점 완료':'미채점',priorityLabels[d.priority1]||'',d.priorityReason||'',agreementLabels[d.agreement]||'',b.renewable??'',b.disaster??'',b.tech??'',b.forest??'',b.transition??'',d.agreementReason||'',countryLabels[d.opposingCountry]||'',compromiseLabels[d.compromiseDimension]||d.compromiseDimension||'',({gradual:'목표는 유지하고 시기를 나눈다',support:'더 감축하는 대신 돈·기술을 지원한다',review:'각자 목표를 정하고 5년마다 높인다'})[d.compromiseChoice]||d.compromiseChoice||'',d.compromise||'',(d.actors||[]).map(k=>actorLabels[k]||k).join(' | '),(d.actors||[]).map(k=>`${actorLabels[k]||k}: ${actorRoleLabels[d.actorAssignments?.[k]]||''}`).join(' | '),d.actorReason||'',d.reconsiderChoice==='revise'?'입장 수정':d.reconsiderChoice?'입장 유지':'',d.reflectionReason||'',d.finalDeclaration||'',s?.elapsedSeconds??'',s?.timeComplianceScore??'',s?.submissionReason||'',ai.total??'',({high:'높음',medium:'보통',low:'낮음'})[ai.confidence]||'',ai.overall||'',ai.strength||'',ai.improvement||'',ai.feedback||'',(ai.reviewFlags||[]).join(' | '),...keys.flatMap(k=>[ai.areas?.[k]?.score??'',ai.areas?.[k]?.reason||'']),...keys.map(k=>sc[k]??''),sc.total??'',s?.teacherNote||''];
 });return {headers,rows};
}
...
function zipStore(files){const locals=[],centrals=[];let offset=0;for(const [name,content] of files){const n=Buffer.from(name),d=Buffer.isBuffer(content)?content:Buffer.from(content),crc=crc32(d),local=Buffer.alloc(30);local.writeUInt32LE(0x04034b50,0);local.writeUInt16LE(20,4);local.writeUInt16LE(0,6);local.writeUInt16LE(0,8);local.writeUInt32LE(crc,14);local.writeUInt32LE(d.length,18);local.writeUInt32LE(d.length,22);local.writeUInt16LE(n.length,26);locals.push(local,n,d);const central=Buffer.alloc(46);central.writeUInt32LE(0x02014b50,0);central.writeUInt16LE(20,4);central.writeUInt16LE(20,6);central.writeUInt32LE(crc,16);central.writeUInt32LE(d.length,20);central.writeUInt32LE(d.length,24);central.writeUInt16LE(n.length,28);central.writeUInt32LE(offset,42);centrals.push(central,n);offset+=local.length+n.length+d.length;}const centralSize=centrals.reduce((a,b)=>a+b.length,0),end=Buffer.alloc(22);end.writeUInt32LE(0x06054b50,0);end.writeUInt16LE(files.length,8);end.writeUInt16LE(files.length,10);end.writeUInt32LE(centralSize,12);end.writeUInt32LE(offset,16);return Buffer.concat([...locals,...centrals,end])}
function buildXlsx(headers,rows){const all=[headers,...rows],last=colName(headers.length-1),sheetRows=all.map((row,ri)=>`<row r="${ri+1}">${row.map((v,ci)=>{const ref=`${colName(ci)}${ri+1}`,numeric=ri>0&&ci!==1&&typeof v==='number'&&Number.isFinite(v);return numeric?`<c r="${ref}" s="3" t="n"><v>${v}</v></c>`:`<c r="${ref}" s="${ri===0?1:2}" t="inlineStr"><is><t xml:space="preserve">${xmlEsc(v)}</t></is></c>`}).join('')}</row>`).join(''),widths=headers.map((h,i)=>`<col min="${i+1}" max="${i+1}" width="${Math.min(42,Math.max(10,String(h).length+4))}" customWidth="1"/>`).join('');const files=[
 ...
 ['xl/workbook.xml','<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="학생 결과·채점" sheetId="1" r:id="rId1"/></sheets></workbook>'],
 ['xl/_rels/workbook.xml.rels','<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>'],
 ['xl/styles.xml','<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="10"/><name val="맑은 고딕"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="맑은 고딕"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF176B45"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="4"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>'],
 ['xl/worksheets/sheet1.xml',`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:${last}${all.length}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${widths}</cols><sheetData>${sheetRows}</sheetData><autoFilter ref="A1:${last}${all.length}"/></worksheet>`]
 ];return zipStore(files)}

function studentPayloadForAI(s){const d=s.data||{};return {country:countryLabels[s.country]||s.country,initialPriority:priorityLabels[d.priority1]||d.priority1||'',priorityReason:d.priorityReason||'',agreement:agreementLabels[d.agreement]||d.agreement||'',budgetAllocation:d.budget||{},policyReason:d.agreementReason||'',opposingCountry:countryLabels[d.opposingCountry]||d.opposingCountry||'',compromiseDimension:compromiseLabels[d.compromiseDimension]||d.compromiseDimension||'',compromise:d.compromise||'',reconsiderChoice:d.reconsiderChoice==='revise'?'일부 판단 수정':'처음 판단 유지',finalDeclaration:d.finalDeclaration||'',actors:(d.actors||[]).map(k=>actorLabels[k]||k),actorAssignments:d.actorAssignments||{},actorReason:d.actorReason||'',reflectionReason:d.reflectionReason||'',timing:{elapsedSeconds:s.elapsedSeconds??null,score:s.timeComplianceScore??null,submissionReason:s.submissionReason||'unknown'}};}
function aiPrompt(s){return `당신은 고등학교 기후위기 수행평가의 교사용 가채점 보조자입니다. 이 과목은 매우 후하게 평가하는 절대평가입니다. 최종 점수는 교사가 결정합니다. 학생이 정답 같은 전문 지식을 썼는지보다 과제를 하려는 의지, 모든 단계의 수행, 질문에 관련된 자기 판단과 이유를 우선 인정하세요. 특정 정책 선택은 정답이나 오답이 아닙니다. 각 영역은 25점, 총 100점입니다.\n1) 국가 이해: 배정 국가의 조건을 파악하고 처음 판단의 이유와 연결했는가.\n2) 정책 선택: 협약과 예산 선택이 국가 조건과 연결되었는가.\n3) 절충안: 상대국을 고려한 행동·지원·양보를 제시했는가.\n4) 성찰·최종 합의: 판단을 돌아보고 실행 방향을 제시했는가.\n반드시 다음 3단계만 사용해 후하게 평가하세요.\n[1단계 성실 수행: 영역별 23~25점] 질문에 관련된 답변을 작성했고 자신의 판단이나 이유가 보이며 과제를 수행하려는 의지가 확인됨. 다소 짧거나 단순하고 표현·맞춤법·전문지식이 부족해도 이 단계입니다. 모든 단계를 성실히 작성한 학생은 특별한 문제가 없다면 각 영역 최소 23점, 총점 92점 이상을 부여하세요. 충실하거나 구체적인 부분이 있으면 24~25점을 적극 부여하세요.\n[2단계 일부 수행: 영역별 18~22점] 관련 답변은 있으나 해당 영역의 핵심 답변이 상당 부분 빠졌거나 지나치게 짧아 판단과 이유를 거의 확인하기 어려움. 그래도 참여 흔적이 있으면 18점 미만으로 내리지 마세요.\n[3단계 미수행: 영역별 0~17점] 빈칸, 의미 없는 문자·반복, 질문과 무관한 내용, 복사한 문구만 있고 자기 답변이 없음, 또는 사실상 수행하지 않은 경우에만 적용하세요.\n점수를 깎을 근거를 찾기보다 성실하게 수행한 근거를 먼저 찾으세요. 문장 표현·맞춤법·전문용어 부족만으로 감점하지 마세요. 점수는 정수로 쓰고 근거문장은 답안에서 60자 이내로 영역당 최대 2개만 인용하세요. 피드백은 격려 중심으로 작성하세요. 반드시 JSON만 출력하세요.\n{"confidence":"high|medium|low","overall":"전체 판단 2~3문장","strength":"잘한 점","improvement":"보완할 점","feedback":"교사용 종합 피드백","reviewFlags":["교사가 확인할 점"],"areas":{"countryUnderstanding":{"score":0,"reason":"","evidence":[]},"policyChoice":{"score":0,"reason":"","evidence":[]},"compromise":{"score":0,"reason":"","evidence":[]},"reflection":{"score":0,"reason":"","evidence":[]}}}\n학생 답안:\n${JSON.stringify(studentPayloadForAI(s),null,2)}`;}
...
function validateAIGrade(raw){const areas={},src=raw?.areas||{};for(const [k,options] of Object.entries(scoreOptions)){const a=src[k]||{},requested=Number(a.score),score=options.includes(requested)?requested:options.reduce((best,v)=>Math.abs(v-requested)<Math.abs(best-requested)?v:best,options[0]);areas[k]={score,reason:String(a.reason||'').slice(0,700),evidence:(Array.isArray(a.evidence)?a.evidence:[]).map(x=>String(x).slice(0,60)).filter(Boolean).slice(0,2)};}const total=Object.values(areas).reduce((a,x)=>a+x.score,0);return {model:OPENAI_MODEL,createdAt:now(),total,confidence:['high','medium','low'].includes(raw?.confidence)?raw.confidence:'medium',overall:String(raw?.overall||'').slice(0,1200),strength:String(raw?.strength||'').slice(0,1000),improvement:String(raw?.improvement||'').slice(0,1000),feedback:String(raw?.feedback||'').slice(0,1200),reviewFlags:(Array.isArray(raw?.reviewFlags)?raw.reviewFlags:[]).map(x=>String(x).slice(0,300)).slice(0,8),areas};}
...
function benchmarkMetrics(rows=[]){const done=rows.filter(x=>Number.isFinite(x.aiScore)),diffs=done.map(x=>x.aiScore-x.expectedScore),major=diffs.filter(x=>Math.abs(x)>5),flagged=done.filter(x=>x.reviewRequired),flaggedMajor=flagged.filter(x=>Math.abs(x.aiScore-x.expectedScore)>5),high=done.filter(x=>x.confidence==='high');return{completed:done.length,failed:rows.filter(x=>x.error).length,mae:done.length?Number((diffs.reduce((a,x)=>a+Math.abs(x),0)/done.length).toFixed(2)):null,within5:done.length?Number((diffs.filter(x=>Math.abs(x)<=5).length/done.length*100).toFixed(1)):null,over:diffs.filter(x=>x>0).length,under:diffs.filter(x=>x<0).length,exact:diffs.filter(x=>x===0).length,reviewPrecision:flagged.length?Number((flaggedMajor.length/flagged.length*100).toFixed(1)):null,reviewCoverage:major.length?Number((flaggedMajor.length/major.length*100).toFixed(1)):null,highConfidenceAccuracy:high.length?Number((high.filter(x=>Math.abs(x.aiScore-x.expectedScore)<=5).length/high.length*100).toFixed(1)):null};}
async function readBenchmark(){try{return JSON.parse(await fsp.readFile(BENCHMARK_FILE,'utf8'))}catch{return null}}
let benchmarkWriteChain=Promise.resolve();async function writeBenchmark(job){const snapshot=JSON.stringify(job,null,2);const write=async()=>{const tmp=BENCHMARK_FILE+'.tmp';await fsp.writeFile(tmp,snapshot,'utf8');await fsp.rename(tmp,BENCHMARK_FILE)};benchmarkWriteChain=benchmarkWriteChain.catch(()=>{}).then(write);return benchmarkWriteChain}
function climateBenchmarkCases(){const levels=[
 {name:'충실',expected:{countryAnalysis:15,fundAllocation:20,policyAgreement:20,conflictAnalysis:15,compromise:10,cooperationRoles:10,completion:4,timeCompliance:6}},
 {name:'대체로 충실',expected:{countryAnalysis:15,fundAllocation:20,policyAgreement:20,conflictAnalysis:15,compromise:8,cooperationRoles:8,completion:4,timeCompliance:6}},
 {name:'부분 수행',expected:{countryAnalysis:15,fundAllocation:16,policyAgreement:20,conflictAnalysis:12,compromise:10,cooperationRoles:10,completion:3,timeCompliance:6}},
 {name:'희소 답안',expected:{countryAnalysis:15,fundAllocation:16,policyAgreement:16,conflictAnalysis:12,compromise:8,cooperationRoles:8,completion:3,timeCompliance:6}},
 {name:'불충분',expected:{countryAnalysis:9,fundAllocation:12,policyAgreement:12,conflictAnalysis:9,compromise:6,cooperationRoles:6,completion:2,timeCompliance:6}}
 ];return Array.from({length:50},(_,i)=>{const level=levels[i%5],student={studentId:`BENCH${String(i+1).padStart(3,'0')}`,className:'검증반',country:countries[i%4]},data=syntheticAnswer(student,i);if(i%5===1){data.compromise='상대국과 일정을 조정하고 서로 협력하겠다.';data.finalDeclaration='선택한 정책을 추진하고 합의 이행을 확인하겠다.'}if(i%5===2){data.priorityReason='우리 국가에 필요하기 때문이다.';data.agreementReason='국가 상황에 맞는다.';data.oppositionReason='나라별 상황이 다르다.';data.compromise='서로 조금씩 양보한다.';data.finalDeclaration='협약을 지키고 국제사회와 협력한다.'}if(i%5===3){data.priorityReason='필요하다.';data.agreementReason='좋다.';data.budget={renewable:100};data.opposingCountry='';data.oppositionReason='';data.compromise='협력한다.';data.finalDeclaration='기후위기를 해결한다.'}if(i%5===4){data.priority1='';data.priorityReason='';data.agreement='';data.budget={};data.agreementReason='';data.opposingCountry='';data.oppositionReason='';data.compromise='';data.finalDeclaration='노력한다.'}data.benchmarkVariation=['책임 중심 표현','피해 중심 표현','경제 중심 표현','지원 중심 표현','이행 중심 표현'][Math.floor(i/10)];const expectedScore=Object.values(level.expected).reduce((a,x)=>a+x,0);return{caseNo:i+1,label:`${level.name} ${Math.floor(i/5)+1}`,session:{country:student.country,data},expected:level.expected,expectedScore,aiScore:null,aiGrade:null,confidence:null,reviewRequired:false,error:null};});}
async function runClimateBenchmark(job){if(job.running)return;job.running=true;job.status='running';job.updatedAt=now();await writeBenchmark(job);let cursor=0;const pending=job.cases.filter(x=>!Number.isFinite(x.aiScore));const workers=Array.from({length:5},async()=>{while(cursor<pending.length){const item=pending[cursor++];item.error=null;try{const grade=await runAIGrade(item.session);item.aiGrade=grade;item.aiScore=Number(grade.total);item.confidence=grade.confidence;item.reviewRequired=Array.isArray(grade.reviewFlags)&&grade.reviewFlags.length>0;}catch(e){item.error=String(e?.message||e).slice(0,500)}item.completedAt=now();job.metrics=benchmarkMetrics(job.cases);job.updatedAt=now();await writeBenchmark(job)}});await Promise.all(workers);job.running=false;job.metrics=benchmarkMetrics(job.cases);job.status=job.metrics.completed+job.metrics.failed>=50?'completed':'paused';job.finishedAt=now();job.updatedAt=now();await writeBenchmark(job);}
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&['/api/save','/api/submit'].includes(url.pathname)){
   const b=await bodyJson(req),student=(await readSessions()).find(x=>x.sessionId===b.sessionId);
   if(!student)return sendJson(res,404,{error:'세션을 찾을 수 없습니다.'});
   const gate=await classSessionWriteState(student),submitting=url.pathname==='/api/submit';
   if(student.status==='submitted')return submitting?sendJson(res,200,{ok:true,alreadySubmitted:true,submittedAt:student.submittedAt,session:student}):sendJson(res,409,{error:'이미 제출이 확정되어 수정할 수 없습니다.',submitted:true});
   const runtime=await readRuntime();if(!runtime.serverOpen)return sendJson(res,423,{error:'학생 입장이 닫혀 있습니다. 교사에게 문의하세요.',serverClosed:true});
   if(!gate.ok)return sendJson(res,423,{error:gate.error,phase:gate.phase});
   if(submitting){
    let elapsed=student.elapsedSeconds??null,score=student.timeComplianceScore??null;
    if(!student.timerExempt){const timer=await timerForSession(student);elapsed=timer.elapsedSeconds??(TOTAL_SECONDS-timer.remainingSeconds);score=timeScore(elapsed);}
    await appendEvent({type:'submit',sessionId:student.sessionId,data:b.data||{},submissionReason:student.timerExempt?'teacher_reopened':'manual',elapsedSeconds:elapsed,timeComplianceScore:score});
    return sendJson(res,200,{ok:true,submittedAt:student.submittedAt,session:student});
   }
  ...
  if(req.method==='GET'&&url.pathname==='/api/teacher/benchmark-50'){const job=await readBenchmark();return sendJson(res,200,{job,metrics:job?.metrics||benchmarkMetrics(job?.cases||[])});}
  if(req.method==='POST'&&url.pathname==='/api/teacher/benchmark-50'){const cfg=await openAIConfig();if(!cfg.configured)return sendJson(res,400,{error:'먼저 OPENAI_API_KEY를 연결하세요.'});const b=await bodyJson(req),previous=await readBenchmark();if(previous?.running&&Date.now()-Date.parse(previous.updatedAt)<15*60*1000)return sendJson(res,409,{error:'이미 50건 검증이 진행 중입니다.'});const job=b.action==='resume'&&previous?{...previous,running:false,status:'paused',cases:previous.cases.map(x=>Number.isFinite(x.aiScore)?x:{...x,error:null})}:{id:newId(),status:'ready',running:false,createdAt:now(),updatedAt:now(),model:cfg.model,cases:climateBenchmarkCases(),metrics:benchmarkMetrics([])};await writeBenchmark(job);setImmediate(()=>runClimateBenchmark(job).catch(async e=>{job.running=false;job.status='paused';job.lastError=String(e?.message||e);job.updatedAt=now();await writeBenchmark(job)}));return sendJson(res,202,{ok:true,message:b.action==='resume'?'미완료 검증을 이어서 실행합니다.':'실제 학생 기록과 분리된 50건 검증을 시작했습니다.',job:{id:job.id,status:'running',total:50}});}
  if(req.method==='GET'&&url.pathname==='/api/teacher/benchmark-50.xlsx'){const job=await readBenchmark();if(!job)return sendJson(res,404,{error:'검증 결과가 없습니다.'});const headers=['번호','유형','기준점수','AI점수','차이','신뢰도','교사 확인 권장','오류'],rows=job.cases.map(x=>[x.caseNo,x.label,x.expectedScore,x.aiScore??'',Number.isFinite(x.aiScore)?x.aiScore-x.expectedScore:'',x.confidence||'',x.reviewRequired?'예':'아니오',x.error||'']),xlsx=buildXlsx(headers,rows);res.writeHead(200,{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Length':xlsx.length,'Cache-Control':'no-store','Content-Disposition':'attachment; filename="climate_benchmark_50.xlsx"'});return res.end(xlsx);}
  if(req.method==='GET'&&url.pathname==='/api/teacher/submission-detail'){const sessionId=String(url.searchParams.get('sessionId')||''),session=(await readStudentSessions()).find(x=>String(x.sessionId)===sessionId);if(!session)return sendJson(res,404,{error:'학생 상세 답안을 찾을 수 없습니다.'});return sendJson(res,200,{session});}
  if(req.method==='GET'&&url.pathname==='/api/teacher/submissions'){const roster=await readRoster(),rt=await readRuntime(),cfg=await openAIConfig(),fullSessions=(await readStudentSessions()).filter(s=>!roster.students.length||roster.students.some(r=>String(r.studentId)===String(s.studentId))).sort((a,b)=>String(a.studentId).localeCompare(String(b.studentId),'ko')),sessions=fullSessions.map(({data,scoreHistory,reopenHistory,aiGradeHistory,aiGrade,...s})=>({...s,aiGrade:aiGrade?{total:aiGrade.total??null,confidence:aiGrade.confidence||''}:null})),presence=readPresence(),classRuntime=await classRuntimeView(),known=new Map();for(const x of roster.students)if(String(x.studentId)!==TEACHER_STUDENT_ID)known.set(String(x.studentId),{studentId:String(x.studentId),name:x.name,className:x.className||deriveClass(x.studentId)});for(const s of sessions)if(!known.has(String(s.studentId)))known.set(String(s.studentId),{studentId:String(s.studentId),name:s.name,className:s.className||deriveClass(s.studentId)});const presenceRows=[...known.values()].map(x=>{const p=presence[x.studentId],last=p?.lastSeen||null,online=!!last&&Date.now()-Date.parse(last)<=25000,studentSessions=sessions.filter(s=>String(s.studentId)===x.studentId);return {...x,online,everEntered:!!(p||studentSessions.length),lastSeen:last,loginCount:Number(p?.loginCount||0),submitted:studentSessions.some(s=>s.status==='submitted'),helpRequest:!!p?.helpRequest,helpCategory:p?.helpCategory||'',helpRequestedAt:p?.helpRequestedAt||null};});return sendJson(res,200,{sessions,roster:{...roster,students:roster.students.filter(x=>String(x.studentId)!==TEACHER_STUDENT_ID)},server:{open:!!rt.serverOpen,updatedAt:rt.updatedAt},timer:{paused:rt.timerPaused,pauseStartedAt:rt.pauseStartedAt},classRuntime,presence:presenceRows,ai:{configured:cfg.configured,model:cfg.model,source:cfg.source,updatedAt:cfg.updatedAt}});}
  if(req.method==='POST'&&url.pathname==='/api/teacher/help/resolve'){const b=await bodyJson(req),studentId=String(b.studentId||''),p=readPresence(),old=p[studentId];if(!old)return sendJson(res,404,{error:'학생 도움 요청을 찾을 수 없습니다.'});p[studentId]={...old,helpRequest:false,helpCategory:'',helpRequestedAt:null,helpResolvedAt:now()};await writePresence();return sendJson(res,200,{ok:true});}
  ...
  if(req.method==='POST'&&url.pathname==='/api/teacher/class-runtime'){
   ...
   if(action==='start'||action==='resume'){if((action==='start'&&rec.phase!=='ready')||(action==='resume'&&!['paused','auto_paused'].includes(rec.phase)))return sendJson(res,409,{error:'현재 상태에서는 시작·재개할 수 없습니다.'});if(!rt.serverOpen)return sendJson(res,409,{error:'전체 학생 서버가 닫혀 있습니다.'});if(rec.phase==='finished')return sendJson(res,409,{error:'이미 45분이 종료된 세션입니다.'});rec.checkpointSeconds=TOTAL_SECONDS;rec.phase='running';rec.runStartedAt=now();rec.updatedAt=now();await writeClassRuntime(cr);}
    ...
    if(['closed','finished'].includes(rec.phase))return sendJson(res,409,{error:'종료 전인 반에만 추가 시간을 부여할 수 있습니다.'});
    const limit=Number(rec.timeLimitSeconds||TOTAL_SECONDS);if(limit>=3300)return sendJson(res,409,{error:'추가 시간은 최대 10분입니다.'});
    rec.timeLimitSeconds=limit+300;rec.updatedAt=now();await writeClassRuntime(cr);
   }
   else if(action==='finish'){rec.elapsedSeconds=classElapsedSeconds(rec);rec.runStartedAt=null;rec.phase='finished';rec.finishReason='teacher_finish';rec.updatedAt=now();await writeClassRuntime(cr);await finalizeClassAnswers(rec);}
  ...
  if(req.method==='POST'&&url.pathname==='/api/teacher/reset-records'){
   const b=await bodyJson(req);if(b.all!==true&&!/^[1-3]반$/.test(String(b.className||'')))return sendJson(res,400,{error:'초기화할 반을 선택하세요.'});
   const all=b.all===true,targets=(await readStudentSessions()).filter(x=>all||(x.className||deriveClass(x.studentId))===b.className);
   const rt=await readRuntime();if(!rt.timerPaused){rt.pauseStartedAt=now();rt.timerPaused=true;}rt.serverOpen=false;await writeRuntime(rt);
   const cr=await readClassRuntime();for(const id of Object.values(cr.classes)){const r=cr.sessions[id];if(r?.phase==='running'){r.elapsedSeconds=classElapsedSeconds(r);r.runStartedAt=null;r.phase='paused';}}
   for(const student of targets)await appendEvent({type:'reset',sessionId:student.sessionId,reason:all?'teacher-reset-all':'teacher-reset-class'});
   const nos=all?[1,2,3]:[Number(b.className.replace('반',''))];for(const n of nos){const rec=classRecord(n);cr.sessions[rec.sessionId]=rec;cr.classes[String(n)]=rec.sessionId;}await writeClassRuntime(cr);
   const presence=readPresence();for(const [id,v] of Object.entries(presence))if(all||v.className===b.className||deriveClass(id)===b.className)delete presence[id];await writePresence();
   return sendJson(res,200,{ok:true,resetCount:targets.length,classRuntime:await classRuntimeView(),serverOpen:false});
  }
  if(req.method==='POST'&&url.pathname==='/api/teacher/ai-grade'){const b=await bodyJson(req),s=(await readSessions()).find(x=>x.sessionId===b.sessionId);if(!s)return sendJson(res,404,{error:'학생 응시를 찾을 수 없습니다.'});if(s.status!=='submitted')return sendJson(res,400,{error:'제출 완료 답안만 AI 가채점할 수 있습니다.'});try{const submittedAt=s.submittedAt;const aiGrade=await runAIGrade(s);return await lifecycleJob(async()=>{const current=(await readSessions()).find(x=>x.sessionId===s.sessionId);if(!current||current.status!=='submitted'||current.submittedAt!==submittedAt)return sendJson(res,409,{error:'가채점 중 답안 상태가 바뀌었습니다. 다시 조회하세요.'});await appendEvent({type:'ai_grade',sessionId:s.sessionId,aiGrade});return sendJson(res,200,{ok:true,aiGrade});});}catch(e){return sendJson(res,503,{error:e.message});}}
  if(req.method==='POST'&&url.pathname==='/api/teacher/score'){
   const b=await bodyJson(req),fields=Object.keys(scoreMax),clean={};if(!b.sessionId||!b.score)return sendJson(res,400,{error:'채점 정보가 부족합니다.'});const student=(await readSessions()).find(x=>x.sessionId===b.sessionId);if(!student)return sendJson(res,404,{error:'초기화되었거나 존재하지 않는 응시입니다. 학생 목록을 다시 확인하세요.'});if(student.status!=='submitted')return sendJson(res,409,{error:'최종 제출된 답안만 채점할 수 있습니다.'});if(!b.expectedUpdatedAt||!Number.isInteger(b.expectedRevision))return sendJson(res,428,{error:'채점 화면을 다시 열어 최신 답안을 확인한 뒤 저장하세요.'});if(b.expectedUpdatedAt!==student.updatedAt||b.expectedRevision!==student.revision)return sendJson(res,409,{error:'다른 화면에서 답안 또는 채점 상태가 변경되었습니다. 상세 답안을 다시 열어 확인하세요.'});
   for(const f of fields){const v=Number(b.score[f]),allowed=scoreOptions[f];if(!allowed.includes(v))return sendJson(res,400,{error:`${scoreLabels[f]} 점수는 ${allowed.join('·')}점 중에서만 선택할 수 있습니다. 공식 채점표상 나올 수 없는 점수입니다.`});clean[f]=v;}clean.total=fields.reduce((a,f)=>a+clean[f],0);if(clean.total<56||clean.total>100)return sendJson(res,400,{error:'공식 채점표상 나올 수 없는 총점입니다.'});await appendEvent({type:'score',sessionId:b.sessionId,score:clean,teacherNote:String(b.teacherNote||'').slice(0,1000)});return sendJson(res,200,{ok:true,score:clean});
  }
...
function lifecycleJob(fn){const job=lifecycleChain.then(fn);lifecycleChain=job.catch(()=>{});return job;}
...
function applyGenerousClimateFloor(grade,session){
 ...
 const order=['countryAnalysis','fundAllocation','policyAgreement','conflictAnalysis','compromise','cooperationRoles','completion'];
...
runAIGrade=async function(session){
 const grade=await runAIGradeBase(session),known=Number.isInteger(session.timeComplianceScore);
 grade.areas.timeCompliance={score:known?session.timeComplianceScore:0,reason:known?`서버 기록: 활동 ${session.elapsedSeconds??0}초 · ${session.submissionReason||'제출'}`:'시간 기록이 없어 교사의 확인이 필요합니다.',evidence:[]};
 if(!known)grade.reviewFlags.push('시간 기록 없음: 교사가 시간 준수 점수를 확인하세요.');
 grade.total=Object.values(grade.areas).reduce((a,v)=>a+v.score,0);
 return applyGenerousClimateFloor(grade,session);
};
