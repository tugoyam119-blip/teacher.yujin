...
function validateAIGrade(raw){const areas={},src=raw?.areas||{};for(const [k,max] of Object.entries(scoreMax)){const a=src[k]||{},score=Math.max(0,Math.min(max,Math.round(Number(a.score)||0)));areas[k]={score,reason:String(a.reason||'').slice(0,700),evidence:(Array.isArray(a.evidence)?a.evidence:[]).map(x=>String(x).slice(0,60)).filter(Boolean).slice(0,2)};}const total=Object.values(areas).reduce((a,x)=>a+x.score,0);return {model:OPENAI_MODEL,createdAt:now(),total,confidence:['high','medium','low'].includes(raw?.confidence)?raw.confidence:'medium',overall:String(raw?.overall||'').slice(0,1200),strength:String(raw?.strength||'').slice(0,1000),improvement:String(raw?.improvement||'').slice(0,1000),feedback:String(raw?.feedback||'').slice(0,1200),reviewFlags:(Array.isArray(raw?.reviewFlags)?raw.reviewFlags:[]).map(x=>String(x).slice(0,300)).slice(0,8),areas};}
