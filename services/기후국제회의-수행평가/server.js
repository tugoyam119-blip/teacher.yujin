...
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&url.pathname==='/api/start'){
   ...
   const rt=await readRuntime(),className=rosterEntry?.className||deriveClass(studentId),classNo=Number(String(className).replace(/\D/g,''));if(classNo<1||classNo>7)return sendJson(res,409,{error:'학생의 반 정보를 확인할 수 없습니다. 교사에게 문의하세요.'});const {rec}=await currentClassRecord(classNo);if(!rec||['closed','finished'].includes(rec.phase))return sendJson(res,423,{error:'우리 반 입장이 아직 열리지 않았습니다. 선생님 안내를 기다려 주세요.',code:'CLASS_ADMISSION_CLOSED'});const country=rosterEntry?.country||fallbackCountry(studentId,className),pauseBaseMs=effectivePausedMs(rt),sessionId=newId();
