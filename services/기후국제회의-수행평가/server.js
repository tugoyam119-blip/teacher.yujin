...
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='GET'&&url.pathname==='/api/teacher/export.csv'){
   const sessions=(await readStudentSessions()).sort((a,b)=>String(a.studentId).localeCompare(String(b.studentId),'ko')),roster=await readRoster();const sessionById=new Map(sessions.map(s=>[s.studentId,s]));const rosterStudents=roster.students.filter(s=>String(s.studentId)!==TEACHER_STUDENT_ID);const base=rosterStudents.length?rosterStudents:sessions.map(s=>({studentId:s.studentId,name:s.name,className:s.className,country:s.country}));
