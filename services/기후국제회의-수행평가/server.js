...
async function teacherExportData(className=''){
 ...
 const officialTeacherHeaders=['교사_국가상황이해관계','교사_기후기금배분','교사_정책협약선택','교사_국가간갈등분석','교사_갈등조정절충안','교사_국제협력주체역할','교사_활동완성도','교사_시간준수','교사_최종총점','교사메모'],officialHeaders=[...headers.slice(0,-6),...officialTeacherHeaders],officialRows=rows.map(row=>{const s=sessionById.get(String(row[1])),sc=s?.score||{};return [...row.slice(0,-6),sc.countryAnalysis??'',sc.fundAllocation??'',sc.policyAgreement??'',sc.conflictAnalysis??'',sc.compromise??'',sc.cooperationRoles??'',sc.completion??'',sc.timeCompliance??'',sc.total??'',s?.teacherNote||'']});const aiKeys=Object.keys(scoreOptions),oldAiStart=officialHeaders.indexOf('AI_국가이해점수');
 officialHeaders.splice(oldAiStart,8,...aiKeys.flatMap(k=>[`AI_${scoreLabels[k]}_점수`,`AI_${scoreLabels[k]}_근거`]));
 officialHeaders.push('판단변화이유','서버경과초','제출확정사유');
 for(const row of officialRows){const student=sessionById.get(String(row[1]));row.splice(oldAiStart,8,...aiKeys.flatMap(k=>[student?.aiGrade?.areas?.[k]?.score??'',student?.aiGrade?.areas?.[k]?.reason||'']));row.push(student?.data?.reflectionReason||'',student?.elapsedSeconds??'',student?.submissionReason||'');}
 return {headers:officialHeaders,rows:className?officialRows.filter(r=>String(r[0])===String(className)):officialRows};
}
...
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&url.pathname==='/api/start'){
   ...
   if(existing&&existing.status==='submitted')return sendJson(res,200,{resumed:true,session:existing,serverNow:now(),timer:await timerForSession(existing)});
