...
function classRecord(classNo,mode='regular'){return {classNo:Number(classNo),mode:mode==='makeup'?'makeup':'regular',sessionId:newId(),phase:'ready',openedAt:now(),runStartedAt:null,elapsedSeconds:0,checkpointSeconds:2700,updatedAt:now()};}
...
async function settleClassRecord(cr,rec){if(!rec||rec.phase!=='running')return rec;const elapsed=classElapsedSeconds(rec),checkpoint=Math.min(TOTAL_SECONDS,Math.max(2700,Number(rec.checkpointSeconds||2700)));if(elapsed<checkpoint)return rec;rec.elapsedSeconds=checkpoint;rec.runStartedAt=null;rec.phase=checkpoint>=TOTAL_SECONDS?'finished':'auto_paused';rec.updatedAt=now();cr.sessions[rec.sessionId]=rec;cr.classes[String(rec.classNo)]=rec.sessionId;await writeClassRuntime(cr);return rec;}
...
async function classRuntimeView(){const cr=await readClassRuntime(),rows=[];for(let classNo=1;classNo<=7;classNo++){const id=cr.classes[String(classNo)],rec=id?cr.sessions[id]:null;if(rec)await settleClassRecord(cr,rec);const live=id?cr.sessions[id]:null,elapsed=live?classElapsedSeconds(live):0;rows.push(live?{...live,elapsedSeconds:elapsed,remainingSeconds:Math.max(0,TOTAL_SECONDS-elapsed)}:{classNo,mode:'regular',sessionId:null,phase:'closed',openedAt:null,runStartedAt:null,elapsedSeconds:0,remainingSeconds:TOTAL_SECONDS,checkpointSeconds:2700,updatedAt:null});}return {version:1,classes:rows};}
...
function touchPresence(s){if(!s?.studentId)return;const p=readPresence(),ts=now();p[String(s.studentId)]={studentId:String(s.studentId),className:s.className||deriveClass(s.studentId),lastSeen:ts,firstSeen:p[String(s.studentId)]?.firstSeen||ts};if(Date.now()-presenceLastWrite>5000){presenceLastWrite=Date.now();fsp.writeFile(PRESENCE_FILE,JSON.stringify(p,null,2),'utf8').catch(()=>{});}}
...
async function handle(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&url.pathname==='/api/start'){
   ...
   const rt=await readRuntime(),className=rosterEntry?.className||deriveClass(studentId),classNo=Number(String(className).replace(/\D/g,''));if(classNo<1||classNo>7)return sendJson(res,409,{error:'학생의 반 정보를 확인할 수 없습니다. 교사에게 문의하세요.'});const {rec}=await currentClassRecord(classNo);if(!rec||!['ready','running','paused','auto_paused'].includes(rec.phase))return sendJson(res,423,{error:`${classNo}반 수행 입장이 아직 열리지 않았습니다. 교사의 안내를 기다리세요.`,classClosed:true});const country=rosterEntry?.country||fallbackCountry(studentId,className),pauseBaseMs=effectivePausedMs(rt),sessionId=newId();
  ...
  if(req.method==='GET'&&url.pathname.startsWith('/api/session/')){const id=url.pathname.split('/').pop(),s=reconstruct(await readEvents()).find(x=>x.sessionId===id);if(!s)return sendJson(res,404,{error:'세션을 찾을 수 없습니다.'});touchPresence(s);return sendJson(res,200,{session:s,serverNow:now(),timer:await timerForSession(s)});}
  if(req.method==='GET'&&url.pathname.startsWith('/api/timer/')){const id=url.pathname.split('/').pop(),s=reconstruct(await readEvents()).find(x=>x.sessionId===id);if(!s)return sendJson(res,404,{error:'세션을 찾을 수 없습니다.'});touchPresence(s);return sendJson(res,200,await timerForSession(s));}

  if(url.pathname.startsWith('/api/teacher/')&&!teacherOK(req,url))return sendJson(res,401,{error:'교사용 비밀번호가 올바르지 않습니다.'});
  if(req.method==='GET'&&url.pathname==='/api/teacher/submissions'){const roster=await readRoster(),rt=await readRuntime(),cfg=await openAIConfig(),sessions=reconstruct(await readEvents()).sort((a,b)=>String(a.studentId).localeCompare(String(b.studentId),'ko')),presence=readPresence(),classRuntime=await classRuntimeView(),known=new Map();for(const x of roster.students)known.set(String(x.studentId),{studentId:String(x.studentId),name:x.name,className:x.className||deriveClass(x.studentId)});for(const s of sessions)if(!known.has(String(s.studentId)))known.set(String(s.studentId),{studentId:String(s.studentId),name:s.name,className:s.className||deriveClass(s.studentId)});const presenceRows=[...known.values()].map(x=>{const p=presence[x.studentId],last=p?.lastSeen||null,online=!!last&&Date.now()-Date.parse(last)<=25000,studentSessions=sessions.filter(s=>String(s.studentId)===x.studentId);return {...x,online,everEntered:!!(p||studentSessions.length),lastSeen:last,submitted:studentSessions.some(s=>s.status==='submitted')};});return sendJson(res,200,{sessions,roster,server:{open:!!rt.serverOpen,updatedAt:rt.updatedAt},timer:{paused:rt.timerPaused,pauseStartedAt:rt.pauseStartedAt},classRuntime,presence:presenceRows,ai:{configured:cfg.configured,model:cfg.model,source:cfg.source,updatedAt:cfg.updatedAt}});}
  ...
  if(req.method==='POST'&&url.pathname==='/api/teacher/class-runtime'){
   ...
   if(action==='start'||action==='resume'){if(!rt.serverOpen)return sendJson(res,409,{error:'전체 학생 서버가 닫혀 있습니다.'});if(rec.phase==='finished')return sendJson(res,409,{error:'이미 총 90분이 종료된 세션입니다.'});if(rec.phase==='auto_paused'&&Number(rec.checkpointSeconds||2700)<TOTAL_SECONDS)rec.checkpointSeconds=TOTAL_SECONDS;rec.phase='running';rec.runStartedAt=now();rec.updatedAt=now();await writeClassRuntime(cr);}
