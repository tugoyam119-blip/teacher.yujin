...
const steps={
  ...
  1:{phase:'국가 입장 설계',title:'자료를 읽고 우리 국가의 조건을 파악하세요',desc:'배출 책임·기후피해·에너지 전환 자료를 비교하고 정책 판단에 중요한 자료 2개를 고릅니다.',notice:'수치가 높고 낮다는 사실만 쓰지 말고, <b>우리 국가의 정책 판단과 연결되는 사실</b>을 찾으세요.'},
  2:{phase:'국가 입장 설계',title:'우리 국가의 우선순위를 정하세요',desc:'경제 성장, 온실가스 감축, 에너지 안정, 기후피해 감소, 국제적 책임 중 1·2순위를 정합니다.',notice:'정답은 없습니다. <b>앞에서 본 자료와 국가 조건</b>이 선택 이유에 들어가면 됩니다.'},
  3:{phase:'국가 입장 설계',title:'국제기후기금 100억 원을 배분하세요',desc:'모든 사업에 기본 10억 원이 있습니다. 남은 50억 원을 5억 원 단위로 추가 배분하세요.',notice:'총액은 반드시 100억 원입니다. <b>최소 한 사업 30억 이상, 한 사업은 10억 유지</b> 조건을 확인하세요.'},
  4:{phase:'국가 입장 설계',title:'예산 선택의 TRADE-OFF를 확인하세요',desc:'무엇을 가장 우선했는지, 예산이 70억 원으로 줄면 무엇을 먼저 줄일지 판단합니다.',notice:'TRADE-OFF는 모든 것을 다 가질 수 없을 때 <b>무엇을 더 중요하게 보고 무엇을 포기하는지</b> 확인하는 과정입니다.'},
  5:{phase:'협상 준비',title:'협상 전 우리 국가의 1차 입장을 확정하세요',desc:'우리 국가의 가장 중요한 목표와 국제사회에 요구할 방향을 2~3문장으로 정리합니다.',notice:'이 입장은 마지막 재판단 단계에서 다시 비교합니다. <b>협상 전 생각을 분명하게</b> 남겨두세요.'},
  6:{phase:'국제협상',title:'새로운 국제기후협약을 선택하세요',desc:'A 동일책임안, B 차등책임안, C 자율감축안 중 우리 국가가 지지할 국제규칙을 선택합니다.',notice:'우리 국가에 유리한지만 보지 말고, <b>다른 국가들과 함께 적용할 수 있는지</b>도 생각하세요.'},
  7:{phase:'국제협상',title:'누가 왜 반대할지 분석하세요',desc:'다른 국가의 조건을 다시 확인하고 우리 정책과 가장 크게 충돌할 국가를 찾습니다.',notice:'기억으로 찍지 말고 <b>상대 국가 카드의 산업·에너지·피해 조건</b>을 근거로 판단하세요.'},
  8:{phase:'국제협상',title:'갈등을 줄일 절충안을 만드세요',desc:'우리 국가와 상대 국가가 모두 일부 수용할 수 있도록 감축률·재정·시기·기술 등의 조건을 조정합니다.',notice:'한쪽만 양보하는 답보다 <b>양쪽이 무엇을 얻고 무엇을 양보하는지</b> 드러나는 절충안이 좋습니다.'},
  9:{phase:'국제협상',title:'국제협력 주체의 역할을 나누세요',desc:'국가·UN·국제환경기구·기업·NGO·시민 중 2개 주체를 골라 서로 다른 역할을 맡깁니다.',notice:'주체의 이름보다 <b>그 주체가 실제로 할 수 있는 역할인지</b> 확인하세요.'},
 10:{phase:'최종 재판단',title:'협상 후 최종 합의를 완성하세요',desc:'협상 전 입장을 다시 보고, 유지할지 수정할지 판단한 뒤 최종 정책 합의를 작성합니다.',notice:'앞 답안을 그대로 반복하기보다 <b>협상 과정에서 새롭게 고려한 점</b>이 최종 판단에 드러나게 하세요.'}
};
...
async function refreshOverview(){if(!teacherKey)return;try{const r=await fetch('/api/teacher/submissions',{headers:{'x-teacher-key':teacherKey},cache:'no-store'});if(!r.ok){if(r.status===401){sessionStorage.removeItem('climateOperateTeacherKey');location.reload();return}throw new Error()}const j=await r.json();sessions=j.sessions||[];serverInfo=j.server||{open:false};timerInfo=j.timer||{paused:!serverInfo.open};renderOverview();renderServerControl();$('#lastUpdated').textContent=new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});$('#syncStatus').textContent='실시간 현황 연결됨';$('#syncStatus').classList.remove('sync-error')}catch{$('#syncStatus').textContent='현황 연결 재시도 중';$('#syncStatus').classList.add('sync-error')}}

function renderServerControl(){const el=$('#operateServerStatus');if(!el)return;el.textContent=serverInfo.open?'🔓 열림 · 학생 수행 가능':'🔒 닫힘 · 학생 대기';el.classList.toggle('timer-stopped',!serverInfo.open);$('#operateCloseServer').disabled=!serverInfo.open;$('#operateOpenServer').disabled=!!serverInfo.open}
async function setAssessmentServer(action){const opening=action==='open';const msg=opening?'수행평가 서버를 열까요? 학생들이 즉시 접속·진행할 수 있습니다.':'수행평가 서버를 닫을까요? 학생 진행과 실제 수행시간이 함께 멈춥니다.';if(!confirm(msg))return;const r=await fetch('/api/teacher/server',{method:'POST',headers:{'Content-Type':'application/json','x-teacher-key':teacherKey},body:JSON.stringify({action})});const j=await r.json().catch(()=>({}));if(!r.ok)return alert(j.error||'서버 상태 변경에 실패했습니다.');serverInfo={open:!!j.open,updatedAt:j.updatedAt};timerInfo={paused:!!j.paused};renderServerControl()}
