...
async function refreshOverview(){if(!teacherKey)return;try{const r=await fetch('/api/teacher/submissions',{headers:{'x-teacher-key':teacherKey},cache:'no-store'});if(!r.ok){if(r.status===401){sessionStorage.removeItem('climateOperateTeacherKey');location.reload();return}throw new Error()}const j=await r.json();sessions=j.sessions||[];timerInfo=j.timer||{paused:false};renderOverview();renderTimerControl();$('#lastUpdated').textContent=new Date().toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit',second:'2-digit'});$('#syncStatus').textContent='실시간 현황 연결됨';$('#syncStatus').classList.remove('sync-error')}catch{$('#syncStatus').textContent='현황 연결 재시도 중';$('#syncStatus').classList.add('sync-error')}}

function renderTimerControl(){const el=$('#operateTimerStatus');if(!el)return;el.textContent=timerInfo.paused?'⏸ 일시정지됨':'▶ 진행 중';el.classList.toggle('timer-stopped',!!timerInfo.paused);$('#operatePauseTimers').disabled=!!timerInfo.paused;$('#operateResumeTimers').disabled=!timerInfo.paused}
async function setStudentTimer(action){const msg=action==='pause'?'쉬는시간 등으로 모든 학생의 실제 수행시간을 일시정지할까요?':'모든 학생의 실제 수행시간을 다시 진행할까요?';if(!confirm(msg))return;const r=await fetch('/api/teacher/timer',{method:'POST',headers:{'Content-Type':'application/json','x-teacher-key':teacherKey},body:JSON.stringify({action})});const j=await r.json().catch(()=>({}));if(!r.ok)return alert(j.error||'시간 제어에 실패했습니다.');timerInfo={...timerInfo,...j};renderTimerControl()}
...
$('#operatePauseTimers').onclick=()=>setStudentTimer('pause');$('#operateResumeTimers').onclick=()=>setStudentTimer('resume');
