...
const RESOURCES=[
 {id:'structure',icon:'🛗',title:'역사 구조와 무단차 이동',summary:'역마다 필요한 추가 대수와 설치비를 구조와 함께 비교하세요.',facts:['새봄중앙역: 추가 3대 · 24억 원','복지센터역: 추가 2대 · 18억 원','대학병원역: 공용 대합실까지 1대, 분리된 상·하행 승강장에 각각 1대가 필요해 총 3대 · 39억 원','전통시장역: 추가 2대 · 20억 원','푸른공원역: 추가 2대 · 18억 원','새봄고역: 기존 대합실↔승강장 엘리베이터를 활용해 지상↔대합실 1대만 추가 · 8억 원'],quiz:{q:'무단차 이동 경로가 완성된 경우는?',c:['역 안에 엘리베이터가 한 대라도 있을 때','지상 출입구부터 승강장까지 계단 없이 연결될 때','승강장에만 엘리베이터가 있을 때'],a:1}},
 {id:'usage',icon:'📊',title:'후보역 이용 현황',summary:'전체 이용객·교통약자 수·비율을 비교하세요.',facts:['새봄중앙역: 하루 18,400명 / 교통약자 1,850명','복지센터역: 하루 8,900명 / 교통약자 1,620명','대학병원역: 하루 15,700명 / 교통약자 2,140명','전통시장역: 하루 11,200명 / 교통약자 1,480명','푸른공원역: 하루 7,200명 / 교통약자 780명','새봄고역: 하루 9,600명 / 교통약자 510명'],quiz:{q:'교통약자 추정 이용객 수가 가장 많은 역은?',c:['새봄중앙역','복지센터역','대학병원역','전통시장역'],a:2}},
 {id:'map',icon:'🗺️',title:'주변 시설과 이동 목적',summary:'역 주변 시설과 이동 목적을 비교하세요.',facts:['새봄중앙역: 새봄시청 320m·환승버스센터 80m','복지센터역: 장애인종합복지관 180m·시립복지지원센터 260m','대학병원역: 새봄대학교병원 220m·응급의료센터','전통시장역: 새봄전통시장 90m·노인복지관 350m','푸른공원역: 푸른공원 120m·생활체육센터 240m','새봄고역: 새봄고등학교 140m·특수학급 통학로'],quiz:{q:'진료·치료 목적 이동과 가장 직접적으로 연결되는 역은?',c:['푸른공원역','전통시장역','대학병원역','새봄고역'],a:2}},
 {id:'voices',icon:'💬',title:'시민의 목소리',summary:'6명의 이동 상황과 구체적인 발언을 비교하세요.',facts:['휠체어 이용 시민: “복지센터 수업이 있는 날에는 계단을 피하려고 두 정거장 전에 내려 택시를 탑니다. 이동시간과 교통비가 매번 더 듭니다.”','대학병원 이용 보호자: “재활치료가 끝난 뒤에는 환자가 더 지쳐 있습니다. 그 상태에서 계단을 오르내리게 하는 것이 가장 걱정됩니다.”','전통시장 상인: “장보기 짐을 든 고령 손님이 계단 앞에서 돌아가는 경우가 있습니다. 시장뿐 아니라 노인복지관 이동도 함께 봐주면 좋겠습니다.”','새봄중앙역 통근 시민: “매일 많은 사람이 환승하는 역도 중요합니다. 다만 이용객 수만으로 정하지 말고 더 절박한 이동이 어디인지도 비교해야 합니다.”','새봄고 학생: “특수학급 친구가 등하교할 때 다른 사람의 도움 없이 학교까지 이어서 이동할 수 있었으면 좋겠습니다.”','유모차 이용 보호자: “공원에 갈 때 아이와 짐을 함께 들고 계단을 이용하기 어렵습니다. 가족 이동도 생활 속 접근성 문제라고 생각합니다.”'],quiz:{q:'시민 의견을 정책 판단에 활용하는 가장 적절한 방법은?',c:['한 시민의 요구를 그대로 정답으로 삼는다','각 시민의 필요를 수치·거리·예산 자료와 함께 비교한다','가장 강하게 말한 의견만 따른다'],a:1}},
 {id:'budget',icon:'💰',title:'올해 사용할 수 있는 예산',summary:'이번 사업에 사용할 수 있는 예산은 최대 50억 원입니다.',facts:['이동편의시설 전체 예산 120억 원','이미 정해진 저상버스·보도정비 사업 70억 원','이번 정책에 사용할 수 있는 예산 50억 원'],quiz:{q:'이번 정책에 사용할 수 있는 최대 예산은?',c:['40억 원','50억 원','70억 원'],a:1}}
];
const STATIONS=[
...
function resourceVisual(id){
 ...
 if(id==='budget')return `<div class="resourceVisual"><div class="notice"><b>정책 판단에서 함께 볼 점</b><br>공공시설은 누구나 실제로 이용할 수 있어야 합니다. 필요한 곳이 많을수록 이동의 긴급성·교통약자 규모·필수시설 접근·지역 형평성·예산 효율을 함께 비교해야 합니다.</div></div>`;
 return '';
}
