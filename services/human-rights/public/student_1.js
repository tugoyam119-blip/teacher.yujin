...
function openZoom(src){const box=document.getElementById('imgZoom'),img=document.getElementById('imgZoomTarget'),stage=document.getElementById('imgZoomStage');if(!box||!img||!stage)return;currentZoom=1;img.onload=function(){const availableW=Math.max(280,stage.clientWidth-28),availableH=Math.max(280,stage.clientHeight-28);zoomBaseScale=Math.min(availableW/img.naturalWidth,availableH/img.naturalHeight,1);applyZoomScale();stage.scrollLeft=0;stage.scrollTop=0};img.src=src;box.classList.add('show');box.setAttribute('aria-hidden','false');document.body.classList.add('zoomOpen')}
