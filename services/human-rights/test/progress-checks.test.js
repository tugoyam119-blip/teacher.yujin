...
test('submitted students go directly to unfinished reflection even when the session is closed',()=>{
 const s=fs.readFileSync(path.join(__dirname,'../public/student_3.js'),'utf8');
 const ctx={visitorMode:false,student:{student_id:'10101'},state:{submitted:true},selfEvaluation:{text:'저장한 초안',submitted_at:null},API_MODE:true,session:{is_open:0},updateReopenStatus:()=>{}};
...
test('public completion opens feedback rather than student self-evaluation',()=>{
 const s=fs.readFileSync(path.join(__dirname,'../public/student_3.js'),'utf8');
 const ctx={visitorMode:true,student:{student_id:'visitor'},state:{submitted:true},API_MODE:true,session:{is_open:1}};
 vm.createContext(ctx);vm.runInContext(s.slice(0,s.lastIndexOf('try{init()}')),ctx);
