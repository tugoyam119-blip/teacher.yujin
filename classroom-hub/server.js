...
function normalizeAnnouncements(input = {}) {
  ...
  const schedules = Array.isArray(source.schedules) ? source.schedules.slice(0, 100).map(row => ({
    ...
    time_range: text(row?.time_range, 60),
...
async function ensureClimateConferenceSchedules() {
  ...
  const rows = [
    ...
    { teacher_name: '박인영', date: '2026-10-01', time_range: '14:50~15:40', class_no: 2, weekday: '목', period: 6 }
  ];
  let changed = false;
  const postponed = announcements.schedules.find(x => x.activity_id === activity.id && x.date === '2026-09-24' && x.grade === 3 && x.class_no === 2);
  if (postponed) { postponed.date = '2026-10-01'; changed = true; }
...
app.get('/health', (req, res) => res.json({ ok: true, name: '유진T 클래스룸', version: '4.8.4', chatgpt_patch_receiver: true, chatgpt_patch_format: 1, storage: pg ? 'postgres' : 'json', github: githubConfigured(), railway: railwayConfigured(), time: iso() }));
...
initStore().then(() => app.listen(PORT, '0.0.0.0', () => console.log(`유진T 클래스룸 v4.8.4 : http://localhost:${PORT}`))).catch(e => { console.error(e); process.exit(1); });
