...
app.get('/health', (req, res) => res.json({ ok: true, name: '유진T 클래스룸', version: '4.7.1', chatgpt_patch_receiver: true, chatgpt_patch_format: 1, storage: pg ? 'postgres' : 'json', github: githubConfigured(), railway: railwayConfigured(), time: iso() }));
...
initStore().then(() => app.listen(PORT, '0.0.0.0', () => console.log(`유진T 클래스룸 v4.7.1 : http://localhost:${PORT}`))).catch(e => { console.error(e); process.exit(1); });
